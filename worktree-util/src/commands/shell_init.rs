...
use clap::{Args, Command, ValueEnum, ArgMatches};
...
pub fn command() -> Command {
    Command::new("shell-init")
        ...
        .args(ShellInitArgs::augment_args(Command::new("shell-init")).get_arguments().cloned().collect::<Vec<_>>())
}

pub fn handle_from_matches(matches: &ArgMatches) {
    let shell = matches.get_one::<Shell>("shell").cloned().unwrap_or(Shell::Fish);
    let navigate = matches.get_one::<String>("navigate").map(|s| s.as_str()).unwrap_or("j");
    let code = matches.get_one::<String>("code").map(|s| s.as_str()).unwrap_or("jc");
